...
import kr.co.voiceware.CUSTOM06;
...
public class JapanSpeechSnowImpl implements ISpeechSpeak {
     ...
     * @return
     */
    public static JapanSpeechSnowImpl newInstance(Context context) {
        return new JapanSpeechSnowImpl(context);
    }
     ...
     * @param ctx
     */
    private JapanSpeechSnowImpl(Context ctx) {
    ...
    private void checkLicense() {
        int rtnCode;
        try {
            rtnCode = context.get().getResources().openRawResource(R.raw.verification_jp_snow).read(license);
        } catch (Exception ex) {
            rtnCode = -1;
        }
        if (rtnCode == -1) {
            CsjlogProxy.getInstance().warn("请确认许可证文件:/res/raw/verification_jp_snow.txt");
        } else {
    ...
    private void checkDBFile() {
        int rtnCode = -1;
        String db_path_detail = Environment.getExternalStorageDirectory().getPath() + "/micro_h16/tts_single_db_jpn_custom06.vtdb";
        String db_path = Environment.getExternalStorageDirectory().getPath() + "/micro_h16/";
        if (copyFile(db_path_detail, db_path)) {
            try {
                rtnCode = CUSTOM06.LOADTTS(db_path, license);
            } catch (Exception e) {
    ...
    private boolean copyFile(String sourceFile, String destFile) {
        ...
        try {
            if (!file.exists()) {
                ...
                InputStream is;
                is = context.get().getAssets().open("tts_single_db_jpn_custom06.vtdb");
    ...
    public void startSpeaking(String text, final OnSpeakListener listener) {
        ...
        try {
            //合成语音文件输出
            rtnCode = CUSTOM06.TextToFile(CUSTOM06.VT_FILE_API_FMT_S16PCM_WAVE, text, filename, -1, -1, -1, -1, -1, -1);
            if (rtnCode == -3) {
                CsjlogProxy.getInstance().warn("请确认许可证文件:/res/raw/verification_jp_snow.txt");
            }
        } catch (Exception e) {
