...
public class ConfInfoUtil {
    ...
    public static void init() {
        ...
        Robot.getInstance().setSnListener(new OnSNListener() {
            @Override
            public void response(String sn) {
                try {
                    ...
                    if (TextUtils.isEmpty(str) || str.contains("empty")) {
                        // sn 未能获取到
                        CsjlogProxy.getInstance().debug("未能获取到SN信息");
                    } else {
                        CsjlogProxy.getInstance().debug("获取到SN信息");
                        ConfInfoUtil.putData(SN_NAME, str);
                        CsjlogProxy.getInstance().debug("从底层获取 SN 成功，SN 为：" + sn);
                        SN = str;
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                    CsjlogProxy.getInstance().debug("发生异常");
                }
//                putData(CONFINFO, SN);
            }
        });
        Robot.getInstance().reqProxy.getSN();
    }
    ...
    public static String getSN() {
        return SN == null ? (String) getData(CONFINFO, SN_NAME) : SN;
    }
