 ...
 * 超市
 */
@Route(path= BRouterPath.MAINPAGE_CHAOSHI)
public class ChaoShiHomeActivity extends BaseMainPageActivity {
...
//        HomeAdapter mAdapter;
    ...
    @BindView(R.id.nav_chaoshi)
    RelativeLayout mNavChemist;
    @BindView(R.id.product_chaoshi)
    RelativeLayout mProductChemist;
    @BindView(R.id.member_center_chaoshi)
    RelativeLayout mMemberCenterChemist;
    @BindView(R.id.peripheral_services_chaoshi)
    RelativeLayout mPeripheralServicesChemist;
    @BindView(R.id.coupon_chaoshi)
    RelativeLayout mCouponChemist;
    @BindView(R.id.entertainment_chaoshi)
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(ChaoShiHomeActivity.this, BatteryService.class)), 1000);
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(ChaoShiHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//        speak(ssb.toString());
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_chaoshi_home,R.layout.vertical_activity_chaoshi_home);
    }
    ...
    @OnClick({R.id.product_chaoshi, R.id.member_center_chaoshi, R.id.peripheral_services_chaoshi, R.id.coupon_chaoshi, R.id.entertainment_chaoshi,R.id.nav_chaoshi})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.product_chaoshi:
                goProductAct();
                break;
            case R.id.member_center_chaoshi:
                goVIPCenterAct();
                break;
            case R.id.peripheral_services_chaoshi:
                goNearByAct();
                break;
            case R.id.coupon_chaoshi:
                goCouponAct();
                break;
            case R.id.entertainment_chaoshi:
                goEntertainmentAct();
                break;
            case R.id.nav_chaoshi:
                goNaviAct();
                break;
        }
    }
...
//                jumpActivity(ProductListActivity.class);
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(ChaoShiHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(ChaoShiHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(ChaoShiHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
