 ...
 * 汽车首页
 */
@Route(path = BRouterPath.MAINPAGE_QICHE)
public class QiCheHomeActivity extends BaseMainPageActivity {
    ...
    @BindView(R.id.nav_yaodian)
    RelativeLayout mNavChemist;
    @BindView(R.id.product_yaodian)
    RelativeLayout mProductChemist;
    @BindView(R.id.member_center_yaodian)
    RelativeLayout mMemberCenterChemist;
    @BindView(R.id.peripheral_services_yaodian)
    RelativeLayout mPeripheralServicesChemist;
    @BindView(R.id.coupon_yaodian)
    RelativeLayout mCouponChemist;
    @BindView(R.id.entertainment_yaodian)
    RelativeLayout mEntertainmentChemist;
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(QiCheHomeActivity.this, BatteryService.class)), 1000);
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(QiCheHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//        speak(ssb.toString());
    ...
    public void sendMoveForward() {
        if(getClass().getName().equals(NaviActivity.class.getName()) ||
                getClass().getName().equals(NaviSettingActivity.class.getName())||
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_qiche_home,R.layout.vertical_activity_qiche_home);
    }
    ...
    @OnClick({R.id.product_yaodian, R.id.member_center_yaodian, R.id.peripheral_services_yaodian, R.id.coupon_yaodian, R.id.entertainment_yaodian, R.id.nav_yaodian})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.product_yaodian:
                goProductAct();
                break;
            case R.id.member_center_yaodian:
                goVIPCenterAct();
                break;
            case R.id.peripheral_services_yaodian:
                goNearByAct();
                break;
            case R.id.coupon_yaodian:
                goCouponAct();
                break;
            case R.id.entertainment_yaodian:
                goEntertainmentAct();
                break;
            case R.id.nav_yaodian:
                goNaviAct();
                break;
        }
    }
...
//        }
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(QiCheHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(QiCheHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(QiCheHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
