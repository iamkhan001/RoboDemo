 ...
 * 银行首页
 */
@Route(path = BRouterPath.MAINPAGE_YINHANG)
public class YinHangHomeActivity extends BaseMainPageActivity {
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(YinHangHomeActivity.this, BatteryService.class)), 1000);
    ...
    public void init() {
        super.init();
        Constants.Scene.CurrentScene = Constants.Scene.YinHangScene;
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(YinHangHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//                            String text = getString(R.string.hello) + "，请问您要办理什么业务？我可以为您介绍和引路。" ;
    ...
    public int getLayoutId() {
        return R.layout.activity_yinhang_home;
    }
    ...
    @OnClick({R.id.nav_yinhang, R.id.product_yinhang, R.id.check_yinhang, R.id.about_yinhang, R.id.personal_center_yinhang, R.id.fun_yinhang, R.id.local_service_yinhang})
...
//                goAboutAct();
    ...
    protected boolean onSpeechMessage(String text, String answerText) {
        ...
        if (text.contains(getString(R.string.business_query))) {
            BRouter.jumpActivityByContent("业务查询");
            return true;
        }

        if (text.contains(getString(R.string.product_introduction))) {
            goProductAct();
            return true;
        }
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(YinHangHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(YinHangHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(YinHangHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
