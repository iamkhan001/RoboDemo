 ...
 * 行政场景主页
 */
@Route(path = BRouterPath.MAINPAGE_XINGZHENG)
public class AdminHomeActivity extends BaseMainPageActivity {
    ...
    private void startHomeService() {
        ...
        new Handler().postDelayed(() -> startService(new Intent(AdminHomeActivity.this, BatteryService.class)), 1000);
    ...
    protected void beforeSetContentView() {
        Constants.Scene.CurrentScene = Constants.Scene.XingZheng;
    }
...
//        SkinCompatManager.getInstance().loadSkin("xingzheng.skins", new SkinCompatManager.SkinLoaderListener() {
//
//            @Override
//            public void onStart() {
...
//
//        prattle("啊");
//        prattle("欢迎来到穿山甲,您问你有什么需求?啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊");
//        setCustomerChatMsg("我想看看有什么优惠。");
//        prattle("请稍等。");
//        setCustomerChatMsg("我想看看服务介绍");
//        prattle("请稍等。");
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(AdminHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//               reOpenThread();
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_admin_home,R.layout.vertical_activity_admin_home);
    }
    ...
    @OnClick({R.id.rl_navi, R.id.rl_service_introduction, R.id.rl_admin_consult, R.id.rl_neaby_service,R.id.rl_vipcenter})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.rl_navi:
                goNaviAct();
                break;
            case R.id.rl_service_introduction:
                goServiceIntroduction();
                break;
            case R.id.rl_admin_consult:
                goAdminConsult();
                break;
            case R.id.rl_neaby_service:
                goNearByAct();
                break;
            case R.id.rl_vipcenter:
                goVipCenter();
                break;
            default:
                    break;
...
//        }
        ...
        if(text.contains("大厅导航")){
            goNaviAct();
        }else if(text.contains("服务介绍")){
            goServiceIntroduction();
        }else if(text.contains("行政咨询")){
            goAdminConsult();
        }else if(text.contains("周边服务")){
            goNearByAct();
        }else if(text.contains("会员中心")){
            getLog().warn("会员中心");
            goVipCenter();
        }
...
//                jumpActivity(ProductListActivity.class);
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(AdminHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(AdminHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(AdminHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
