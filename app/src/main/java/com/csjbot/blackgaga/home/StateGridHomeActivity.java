 ...
 * 国家电网首页
 */
@Route(path = BRouterPath.MAINPAGE_STATE_GRID)
public class StateGridHomeActivity extends BaseMainPageActivity {
    ...
    private void startHomeService() {
        ...
        new Handler().postDelayed(() -> startService(new Intent(StateGridHomeActivity.this, BatteryService.class)), 1000);
...
//        SkinCompatManager.getInstance().loadSkin("test_power.skins", new SkinCompatManager.SkinLoaderListener() {
//
//            @Override
//            public void onStart() {
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(StateGridHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//               reOpenThread();
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_state_grid_home,R.layout.vertical_activity_state_grid_home);
    }
    ...
    @OnClick({R.id.iv_navi, R.id.iv_service_introduction, R.id.iv_power_consult, R.id.iv_nearby_service,R.id.iv_vipcenter})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.iv_navi:
                goNaviAct();
                break;
            case R.id.iv_service_introduction:
                goServiceIntroduction();
                break;
            case R.id.iv_power_consult:
                goPowerConsult();
                break;
            case R.id.iv_nearby_service:
                goNearByAct();
                break;
            case R.id.iv_vipcenter:
                goVipCenter();
                break;
            default:
                    break;
...
//                jumpActivity(ProductListActivity.class);
    ...
    public void goPowerConsult(){
        BRouter.jumpActivityByContent("电力咨询");
    }
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(StateGridHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(StateGridHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(StateGridHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
