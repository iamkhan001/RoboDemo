 ...
 * 医院首页
 */
@Route(path= BRouterPath.MAINPAGE_YIYUAN)
public class YiYuanHomeActivity extends BaseMainPageActivity {
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(YiYuanHomeActivity.this, BatteryService.class)), 1000);
...
//        menus.add(menu);
        ...
        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.nav_way));
        ...
        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.hospital_introduce));
...
//        getLog().debug("class:HomeActivity message:homepage data is null, use default data");
    ...
    List<RobotMenuListBean.ResultBean.MenulistBean> loadMenu() {
        ...
        if (robotMenuListBean != null) {
            RobotMenuListBean.ResultBean resultBean = (RobotMenuListBean.ResultBean) robotMenuListBean.getResult();
            List<RobotMenuListBean.ResultBean.MenulistBean> menus = (List<RobotMenuListBean.ResultBean.MenulistBean>) resultBean.getMenulist();
            ...
            for (RobotMenuListBean.ResultBean.MenulistBean menu : menus) {
                if (menu.getLevel()==1) {
                    i++;
                    datas.add(menu);
                    if (menu.getIsdefaulthome().toString().equals("1")) {
                        centerIndex = i;
                    }
                }
            }
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(YiYuanHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//        speak(ssb.toString());
    ...
    private OnItemClickListener itemClickListener = new OnItemClickListener() {
        ...
        public void itemClick(int position) {
            ...
            if (name.equals(getString(R.string.nav_way))) {
...
//                actClass = EntertainmentActivity.class;
            } else if (name.equals(getString(R.string.hospital_introduce))) {
                BRouter.sync(getString(R.string.hospital_introduce));
...
//                actClass = VipCenterActivity.class;
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_home_shangsha,R.layout.vertical_activity_home);
    }
        ...
        public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (BuildConfig.FLAVOR.equals(BuildConfig.ROBOT_TYPE_DEF_ALICE_PLUS)
                ...
                return new MyViewHolder(LayoutInflater.from(
                        YiYuanHomeActivity.this).inflate(R.layout.vertical_item_yiyuan_list, parent,
                        false));
            } else {
                return new MyViewHolder(LayoutInflater.from(
                        YiYuanHomeActivity.this).inflate(R.layout.item_yiyuan_list, parent,
                        false));
            }
        }
...
//        NaviTextHandler.init(this);
        ...
        if (text.contains(getString(R.string.nav_way))) {
            BlackgagaLogger.debug("isloadMap" + isLoadMapSuccess);
            if (!isLoadMapSuccess) {
                restoreMapDialog();
            } else {
            ...
            BRouter.sync(getString(R.string.consultation_service));
            return true;
        } else if (text.contains(getString(R.string.hospital_introduce)) || text.contains("介绍一下你们的医院")) {
            BRouter.sync(getString(R.string.hospital_introduce));
            return true;
        }
...
//        }
    ...
    private void checkConnect() {
        mRecyclerView.postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(YiYuanHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(YiYuanHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(YiYuanHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
