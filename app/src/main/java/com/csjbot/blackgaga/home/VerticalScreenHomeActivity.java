...
import com.csjbot.blackgaga.widget.MyVideoView;
 ...
 * 竖屏主页
 */
@Route(path = BRouterPath.MAINPAGE_VERTICALSCREEN)
public class VerticalScreenHomeActivity extends BaseMainPageActivity {
    ...
    @BindView(R.id.main_page_video)
    MyVideoView main_page_video;
    ...
    public void afterViewCreated(Bundle savedInstanceState) {
        ...
        SkinCompatManager.getInstance().loadSkin("alice_plus.skins", new SkinCompatManager.SkinLoaderListener() {
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(VerticalScreenHomeActivity.this, BatteryService.class)), 1000);
    ...
    public void networkAvailability() {
        ...
        com.csjbot.blackgaga.model.http.factory.ServerFactory.createAdvertisement().getAdvertisement(sn, Constants.Language.getLanguageStr(), new Observer<AdvBean>() {
            @Override
            ...
            public void onNext(@NonNull AdvBean advBean) {
                getLog().info("getAdvertisement onNext");
                if (advBean != null && advBean.getResult() != null) {
                    getLog().info("getAdvertisement onNext:advBean" + new Gson().toJson(advBean));
                    mVideos = new ArrayList<>();
                    for (int i = 0; i < advBean.getResult().size(); i++) {
                        if (TextUtils.equals("video", advBean.getResult().get(i).getAdvtype())) {
                            mVideos.add(advBean.getResult().get(i).getAdvurl());
                        } else if (TextUtils.equals("audio", advBean.getResult().get(i).getAdvtype())) {
//                            mListMusic.add(mAdvBean.getResult().get(i).getAdvurl());
                        } else if (TextUtils.equals("picture", advBean.getResult().get(i).getAdvtype())) {
...
//                            mListImage.add(mAdvBean.getResult().get(i).getAdvurl());
            ...
            public void onError(@NonNull Throwable e) {
                startVideo();
                getLog().info("getAdvertisement onError");
            }
    ...
    private void startVideo(){
        main_page_video.setOnPreparedListener(mp -> {
            mMediaPlayer = mp;
            mp.setVideoScalingMode(MediaPlayer.VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING);
            mp.setVolume(0f,0f);
        });
        main_page_video.setOnCompletionListener(mp -> {
            if(mVideos == null || mVideos.size() == 0){
                Uri uri = Uri.parse("android.resource://" + getPackageName() + "/raw/" + R.raw.vertical_default);
                main_page_video.setVideoURI(uri);
                main_page_video.start();
                return;
            }
            mVideoIndex++;
            if(mVideos.size() > mVideoIndex){

            }else{
                mVideoIndex = 0;
            }
            main_page_video.setVideoPath(getProxyUrl(mVideos.get(mVideoIndex)));
            main_page_video.start();
        });
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(VerticalScreenHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
    ...
    protected void onResume() {
        super.onResume();
        if (onPause) {
            getLog().info("Main_Page_Vide_RESUME");
            main_page_video.start();
        }
        onPause = false;
        isWelcome = true;
...
//                            String text = "哈喽，"+name+"，请问您要办理什么业务？" ;
                ...
                if (person) {
                    ...
                    new Thread(() -> {
                        ...
                        if (!isComplete && isWelcome) {
                            ...
                            isComplete = true;
//                            String text = getString(R.string.hello) + "，请问您要办理什么业务？我可以为您介绍和引路。" ;
    ...
    @OnClick({R.id.nav_yinhang, R.id.product_yinhang, R.id.check_yinhang, R.id.personal_center_yinhang, R.id.fun_yinhang, R.id.local_service_yinhang})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.nav_yinhang:
                goNaviAct();
                break;
            case R.id.product_yinhang:
//                CSJToast.showToast(this,"模块开发中");
                goProductAct();
//                goProductWithoutShoppingCartAct();
                break;
            case R.id.check_yinhang:
                Toast.makeText(context, "模块开发中", Toast.LENGTH_SHORT).show();
//                   CSJToast.showToast(this,"模块开发中");
//                goSearchAct();
//                BRouter.jumpActivityByContent("业务查询");
                break;
//            case R.id.about_yinhang:
//                Toast.makeText(context, "模块开发中", Toast.LENGTH_SHORT).show();
////                CSJToast.showToast(this,"模块开发中");
////                goAboutAct();
////                BRouter.jumpActivityByContent("关于银行");
//                break;
            case R.id.personal_center_yinhang:
                goVIPCenterAct();
                break;
            case R.id.fun_yinhang:
                goEntertainmentAct();
                break;
            case R.id.local_service_yinhang:
                goNearByAct();
                break;
        }
    }
    ...
    protected boolean onSpeechMessage(String text, String answerText) {
        ...
        if (text.contains(getString(R.string.about)) || text.contains(getString(R.string.company_introduction))
                || text.contains(getString(R.string.aboutus))) {
            Toast.makeText(context, "模块开发中", Toast.LENGTH_SHORT).show();
            return true;
        }else if(text.contains(getString(R.string.fun))){
            goEntertainmentAct();
        }else if(text.contains(getString(R.string.product))){
            goProductAct();
        }else if(text.contains(getString(R.string.local_service))){
            goNearByAct();
        }else if(text.contains(getString(R.string.vip_center))){
            goVIPCenterAct();
        }else if(text.contains(getString(R.string.nav_service))){
            goNaviAct();
        }
...
//        BRouter.jumpActivity(BRouterPath.COUPON);
    }
...
//        BRouter.jumpActivity(BRouterPath.ABOUT_US_PATH);
    }

    public void goSearchAct() {
        jumpActivity(Content3Activity.class);
//        BRouter.jumpActivity(BRouterPath.THERELEVEL);
    }
...
//        BRouter.jumpActivity(BRouterPath.VIP_CENTER);
    ...
    public void goProductWithoutShoppingCartAct() {
        jumpActivity(ProductListWithoutShoppingCartActivity.class);
    }
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(VerticalScreenHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(VerticalScreenHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(VerticalScreenHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
