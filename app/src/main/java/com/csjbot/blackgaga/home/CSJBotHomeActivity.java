 ...
 * 穿山甲首页
 */
@Route(path = BRouterPath.MAINPAGE_CSJBOT)
public class CSJBotHomeActivity extends BaseMainPageActivity {
    ...
    @BindView(R.id.nav_csj)
    ImageView mNavChemist;
    @BindView(R.id.product_csj)
    ImageView mProductChemist;
    @BindView(R.id.member_center_csj)
    ImageView mMemberCenterChemist;
    @BindView(R.id.peripheral_services_csj)
    ImageView mPeripheralServicesChemist;
    @BindView(R.id.coupon_csj)
    ImageView mCouponChemist;
    @BindView(R.id.entertainment_csj)
    ImageView mEntertainmentChemist;
...
//        startService(new Intent(this, GetMessageService.class));
        ...
        new Handler().postDelayed(() -> startService(new Intent(CSJBotHomeActivity.this, BatteryService.class)), 1000);
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(CSJBotHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
...
//        speak(ssb.toString());
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_csjbot_home,R.layout.vertical_activity_csjbot_home);
    }
    ...
    @OnClick({R.id.product_csj, R.id.member_center_csj, R.id.peripheral_services_csj, R.id.coupon_csj, R.id.entertainment_csj, R.id.nav_csj})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.product_csj:
//                BRouter.jumpActivityByContent("业务查询");
                goProductAct();
                break;
            case R.id.member_center_csj:
                goVIPCenterAct();
                break;
            case R.id.peripheral_services_csj:
                goNearByAct();
                break;
            case R.id.coupon_csj:
                goCouponAct();
                break;
            case R.id.entertainment_csj:
                goEntertainmentAct();
                break;
            case R.id.nav_csj:
                goNaviAct();
...
//                jumpActivity(ProductListActivity.class);
    ...
    private void checkConnect() {
        new Handler().postDelayed(() -> {
            runOnUiThread(() -> {
                if (!isFinish) {
                    Toast.makeText(CSJBotHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void restoreMapResult(boolean isState) {
        runOnUiThread(() -> {
            ...
            if (isState) {
                ...
                Toast.makeText(CSJBotHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                jumpActivity(NaviActivity.class);
            } else {
                Toast.makeText(CSJBotHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
            }
        });
    }
