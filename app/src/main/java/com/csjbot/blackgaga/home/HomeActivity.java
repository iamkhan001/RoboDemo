...
@Route(path = BRouterPath.HOME_PAGE)
public class HomeActivity extends BaseMainPageActivity {
    ...
    private boolean isNaviFromAI = false;
    private boolean isPointKnow = false;
...
//        CSJToast.showToast(this, Logo.getLogoName()+"");
    }
...
//    void addDetectPersonListener() {
//        RobotManager.getInstance().addListener((OnDetectPersonListener) state -> {
...
//        }
                ...
                for (String keyWord : Constants.NearbyKeyWord.keyWords) {
                    if (text.contains(keyWord)) {
                        BRouter.getInstance()
                                ...
                                .withString(BRouterKey.POISEARCH_KEYWORD, keyWord)
                                .navigation();
                        return true;
                    }
                }
    ...
    private boolean productIntent(String text) {
        CsjlogProxy.getInstance().info("ProductIntent: products:" + Constants.ProductKeyWord.products.size());
        for (String str : Constants.ProductKeyWord.intents) {
            if (text.contains(str)) {
                if (Constants.ProductKeyWord.products == null || Constants.ProductKeyWord.products.size() == 0) {
                    Constants.ProductKeyWord.initKeywords();
                }
...
//                jumpActivity(ProductListActivity.class);
    ...
    public void goNaviAct() {
        BlackgagaLogger.debug("goNaviAct");
        if (!isLoadMapSuccess) {
            restoreMapDialog();
        } else {
