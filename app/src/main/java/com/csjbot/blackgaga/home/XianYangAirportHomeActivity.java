...
import com.csjbot.blackgaga.feature.entertainment.xianyang.XianYangEntertainmentActivity;
...
import com.csjbot.blackgaga.service.WeatherService;
 ...
 * 咸阳机场首页
 */
@Route(path= BRouterPath.MAINPAGE_JICHANG)
public class XianYangAirportHomeActivity extends BaseMainPageActivity {
...
//        startService(new Intent(this, GetMessageService.class));
        if (!(BuildConfig.FLAVOR.equals(BuildConfig.ROBOT_TYPE_DEF_ALICE_PLUS)
            ...
            new Handler().postDelayed(()->startService(new Intent(this, WeatherService.class)),1000);
        }

        loadData();

        new Handler().postDelayed(() -> startService(new Intent(XianYangAirportHomeActivity.this, BatteryService.class)), 1000);
    ...
    private void initNotice(){
        com.csjbot.blackgaga.model.http.factory.ServerFactory.createNoticeSerivce().getGlobalAnnouncement(ProductProxy.SN, new Observer<NoticeBean>() {
            ...
            public void onNext(@NonNull NoticeBean noticeBean) {
                ...
                if(noticeBean != null && noticeBean.getResult() != null){
                    if(noticeBean.getResult().isEnabled()) {
                        ...
                        if(System.currentTimeMillis() < endTime){
                            ...
                            Intent intent = new Intent(XianYangAirportHomeActivity.this,GlobalNoticeService.class);
                ...
                startService(new Intent(XianYangAirportHomeActivity.this, GlobalNoticeService.class));
    ...
    List<RobotMenuListBean.ResultBean.MenulistBean> getDefaultMenu() {
        ...
        menu.setMenuName(getString(R.string.vip_services));
        menu.setMImg(String.valueOf(R.drawable.iv_service_introduction));
        menus.add(menu);

        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.personal_center));
        menu.setMImg(String.valueOf(R.drawable.iv_member_center));
        menus.add(menu);

        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.vipnav));
        ...
        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.happy_time));
        menu.setMImg(String.valueOf(R.drawable.iv_entertainment));
        menus.add(menu);

        menu = new RobotMenuListBean.ResultBean.MenulistBean();
        menu.setMenuName(getString(R.string.aboutus));
    ...
    List<RobotMenuListBean.ResultBean.MenulistBean> loadMenu() {
        ...
        if (robotMenuListBean != null) {
            List<RobotMenuListBean.ResultBean.MenulistBean> menus =
                    com.csjbot.blackgaga.model.http.factory.ServerFactory.createProduct().getMenuList().getResult().getMenulist();
...
//            }
    ...
    public void init() {
        super.init();
        Constants.Scene.CurrentScene = Constants.Scene.XianYangJiChang;
            ...
            if (current != null) {
                speak(current.getName() + getString(R.string.destination), new OnSpeakListener() {
                    ...
                    public void onCompleted(SpeechError speechError) {
                        startActivity(new Intent(XianYangAirportHomeActivity.this, NaviGuideCommentActivity.class));
                    }
                });
            }
        }
                    ...
                    try {
                        ...
                        if (confidence > 80) {
                            ...
                            if(Constants.Language.isChinese()) {
                                text = "尊敬的" + name + "旅客您好，欢迎光临" + Logo.getLogoName() + "，我们已为您准备好休息室，这边请！";
                            }else{
                                text = getString(R.string.hello) + "," + name + "," + getString(R.string.welcome) + Logo.getLogoName() + "，" + getString(R.string.happy_service);
                            }
                            welcome(text);
                ...
                if (person) {
                    ...
                    new Thread(() -> {
                        ...
                        if (!isComplete && isWelcome) {
                            ...
                            String text = "";
                            if(Constants.Language.isChinese()) {
                                text = "尊敬的旅客您好，欢迎光临" + Logo.getLogoName() + "，我们已为您准备好休息室，这边请！";
                            }else{
                                text = getString(R.string.hello_welcome) + Logo.getLogoName() + "，" + getString(R.string.happy_service);
                            }
                            welcome(text);
                            welcomeAction();
                        }
...
//        speak(ssb.toString());
    ...
    private OnItemClickListener itemClickListener = new OnItemClickListener() {
        ...
        public void itemClick(int position) {
            ...
            if (name.equals(getString(R.string.vipnav))) {
            ...
            } else if (name.equals(getString(R.string.happy_time))) {
//                actClass = EntertainmentActivity.class;
                actClass = XianYangEntertainmentActivity.class;
            } else if (name.equals(getString(R.string.vip_services))) {
              BRouter.sync(getResources().getString(R.string.vip_services));
//                actClass = XianYangVipHallActivity.class;
            } else if (name.equals(getString(R.string.local_service))) {
                actClass = NearByAirportActivity.class;
            } else if (name.equals(getString(R.string.personal_center))) {
                actClass = VipCenterActivity.class;
            } else if (name.equals(getString(R.string.aboutus))) {
                BRouter.sync(getResources().getString(R.string.aboutus));
                return ;
...
//                actClass = AboutUsActivity.class;
    ...
    class HomeAdapter extends RecyclerView.Adapter<HomeAdapter.MyViewHolder> {
        ...
        public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (BuildConfig.FLAVOR.equals(BuildConfig.ROBOT_TYPE_DEF_ALICE_PLUS)
                    || BuildConfig.FLAVOR.equals(BuildConfig.ROBOT_TYPE_DEF_AMY_PLUS)) {
                return new MyViewHolder(LayoutInflater.from(XianYangAirportHomeActivity.this)
                        .inflate(R.layout.vertical_item_airporthome_list, parent, false));
            } else {
                return new MyViewHolder(LayoutInflater.from(XianYangAirportHomeActivity.this)
                        .inflate(R.layout.item_airporthome_list, parent, false));
            }
        }
...
//        }
        ...
        if(text.contains(getString(R.string.read_a_read))){
            jumpActivity(ReadActivity.class);
            return true;
        }

        if(childPageCheck(text)){
            return true;
        }
    ...
    private boolean childPageCheck(String text){
        Class<? extends Activity> activityClass = null;
        if (text.contains(getString(R.string.vipnav))) {
            BlackgagaLogger.debug("isloadMap" + isLoadMapSuccess);
            if (!isLoadMapSuccess) {
                restoreMapDialog();
            } else {
                activityClass = NaviActivity.class;
            }
        } else if (text.contains(getString(R.string.happy_time))) {
            activityClass = XianYangEntertainmentActivity.class;
        } else if (text.contains(getString(R.string.vip_services))) {
            BRouter.sync(getString(R.string.vip_services));
            return true;
        } else if (text.contains(getString(R.string.local_service))) {
            activityClass = NearByAirportActivity.class;
        } else if (text.contains(getString(R.string.personal_center))) {
            activityClass = VipCenterActivity.class;
        } else if(text.contains(getString(R.string.aboutus))){
            BRouter.sync(getResources().getString(R.string.aboutus));
            return true;
        }else{
            return false;
        }
        jumpActivity(activityClass);
        return true;
    }
    ...
    private void checkConnect() {
        mRecyclerView.postDelayed(() -> {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (!isFinish) {
                        Toast.makeText(XianYangAirportHomeActivity.this, R.string.check_linux, Toast.LENGTH_SHORT).show();
                    }
                }
            });
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (isState) {
                    ...
                    Toast.makeText(XianYangAirportHomeActivity.this, R.string.restore_map_success, Toast.LENGTH_LONG).show();
                    jumpActivity(NaviActivity.class);
                } else {
                    Toast.makeText(XianYangAirportHomeActivity.this, R.string.restore_map_fail, Toast.LENGTH_LONG).show();
                }
            }
        });
    }
